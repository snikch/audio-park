(() => {
  ...
  if (['control', 'prototype', 'validation', 'interference', 'pcb', 'commissioning'].includes(station)) {
  ...
  function runAdvancedStation(kind) {
    ...
    const profile = {
      ...
      interference: { key: "audioPark.interferenceCanyon.v1", prerequisite: "audioPark.validationObservatory.v1", stop: 9, title: "Interference Canyon", lead: "Trace one named source, path, victim, and return before selecting a model layout response.", chapters: ["Canyon map", "Coupling terms", "Return rule", "Observe & predict", "Retrieve & decide", "Optional bench extension"], first: "mechanism", second: "return", design: "map", target: { x: 854, y: 112 }, next: "PCB Foundry" },
      pcb: { key: "audioPark.pcbFoundry.v1", prerequisite: "audioPark.interferenceCanyon.v1", stop: 10, title: "PCB Foundry", lead: "Turn stated project inputs into a reviewable package without treating CAD output as physical proof.", chapters: ["Foundry map", "Input ledger", "Review rule", "Observe & predict", "Retrieve & decide", "Optional bench extension"], first: "inputs", second: "review", design: "trace", target: { x: 625, y: 106 }, next: "Commissioning Garden" },
      commissioning: { key: "audioPark.commissioningGarden.v1", prerequisite: "audioPark.pcbFoundry.v1", stop: 11, title: "Commissioning Garden", lead: "Plan staged comparisons and a safe stop decision without purchasing, assembling, or energising hardware.", chapters: ["Garden map", "Bring-up terms", "Stop rule", "Observe & predict", "Retrieve & decide", "Optional bench extension"], first: "staged", second: "comparison", design: "defer", target: { x: 404, y: 126 }, next: "Released course complete" },
    }[kind];
    ...
    const defaults = { chapter: 0, maxChapter: 0, completed: [false, false, false, false, false, false], model: { value: kind === "control" ? "offline" : kind === "prototype" ? "unknown" : kind === "interference" ? "sharedReturn" : kind === "pcb" ? "ruleInput" : kind === "commissioning" ? "stopCondition" : "geometry" }, prediction: { answer: "", reasoning: "", revealed: false }, retrieval: { first: "", second: "" }, design: { choice: "", reasoning: "" }, lab: { status: "not-started", evidence: "", safety: false } };
    ...
    const correctPrediction = () => kind === "control" ? (state.model.value === "offline" ? "local" : "remote") : kind === "prototype" ? (state.model.value === "unknown" ? "external" : "model") : kind === "interference" ? (state.model.value === "sharedReturn" ? "map" : "guess") : kind === "pcb" ? (state.model.value === "ruleInput" ? "trace" : "default") : kind === "commissioning" ? (state.model.value === "stopCondition" ? "stop" : "advance") : (state.model.value === "geometry" ? "review" : "simulate");
    const label = (value) => ({ offline: "remote unavailable", online: "remote available", unknown: "unknown module boundary", supported: "supported model boundary", geometry: "routing/clearance risk", circuit: "circuit behaviour risk", sharedReturn: "shared return path", fieldPath: "electric or magnetic field path", ruleInput: "project input and rule", reviewLimit: "review limitation", stopCondition: "stop condition", comparisonStage: "bounded comparison stage" })[value];
    ...
    function updateRoutes() { all.forEach(([, route, building, url]) => { setAccess(route, building, true, url === `?station=${kind}`, $(building).querySelector("text").textContent.replace(/^\d+\.\s*/, "")); }); $("routeStatus").textContent = "11 stations open"; }
    ...
    function render() { renderNav(); renderFooter(); $("lessonContent").innerHTML = template(); if (state.chapter === 3 && ["interference", "pcb", "commissioning"].includes(kind)) { const choices = kind === "interference" ? { base: "sharedReturn", alt: "fieldPath", good: "map", bad: "guess", baseText: "Shared return path", altText: "Electric or magnetic field path", goodText: "Map the named path before changing layout", badText: "Apply a generic ground rule" } : kind === "pcb" ? { base: "ruleInput", alt: "reviewLimit", good: "trace", bad: "default", baseText: "Project input and rule", altText: "Review limitation", goodText: "Trace the input, limit, check, and review", badText: "Trust a default CAD report" } : { base: "stopCondition", alt: "comparisonStage", good: "stop", bad: "advance", baseText: "Stop condition", altText: "Bounded comparison stage", goodText: "Stop and retain the condition", badText: "Advance to hardware anyway" }; $("modelControl").innerHTML = `<option value="${choices.base}">${choices.baseText}</option><option value="${choices.alt}">${choices.altText}</option>`; $("modelControl").value = state.model.value; document.querySelector(".answer-list").innerHTML = `<label><input type="radio" name="prediction" value="${choices.good}" ${state.prediction.answer === choices.good ? "checked" : ""}/> ${choices.goodText}</label><label><input type="radio" name="prediction" value="${choices.bad}" ${state.prediction.answer === choices.bad ? "checked" : ""}/> ${choices.badText}</label>`; } bind(); updateWorld(); }
