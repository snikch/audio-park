(() => {
  ...
  function runAdvancedStation(kind) {
    ...
    const prerequisite = kind === "control" ? powerCurrent(read(profile.prerequisite)) : kind === "prototype" ? controlCurrent(read(profile.prerequisite)) : prototypeCurrent(read(profile.prerequisite));
    if (!prerequisite) { location.href = location.pathname; return; }
    ...
    function updateRoutes() { const controlReady = kind === "control" ? current() : controlCurrent(read("audioPark.controlTower.v1")); const prototypeReady = kind === "prototype" ? current() : prototypeCurrent(read("audioPark.prototypeWorkshop.v1")); all.forEach(([, route, building, url], index) => { const ready = index < profile.stop - 1 || (index === 5 ? (kind === "control" || controlReady) : index === 6 ? (kind === "prototype" || controlReady) : index === 7 ? (kind === "validation" || prototypeReady) : false); setAccess(route, building, ready, url === `?station=${kind}`, $(building).querySelector("text").textContent.replace(/^\d+\.\s*/, "")); }); const released = kind === "control" ? 6 + Number(current()) : kind === "prototype" ? 7 + Number(current()) : 8; $("routeStatus").textContent = `${released} stations open`; }
